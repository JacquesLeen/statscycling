...
nlaps= 7 #FILL WITH NLAPS 
lap_length = 2650 #FILL WITH LAP LENGTH

## ================================================================================##
#Data frame manip -> conversion to seconds and creation of timegaps
#install.packages("lubridate")
library(lubridate)
DF_LapTime= DF[-c(9:10),]
#for (i in 2:6) {
#  DF_LapTime[[i]]= period_to_seconds(hms(DF_LapTime[[i]]))
#}

DF_For_Time_Gap=DF[-c(9:10),]
for(i in 2:11)
  DF_For_Time_Gap[[i]]= period_to_seconds(hms(DF_LapTime[[i]]))
# no start loop
DF_LapTime_NSL= DF_LapTime[-1,]

#timegaps
DF_TimeGap = DF_For_Time_Gap[1:6]
for (j in 1:nlaps+1) {
  temp = DF_TimeGap[j,2]
  for (t in 2:6) {
    if(j == 1){
...
library(reshape2)
DFMelt = melt(DF_LapTime[,c(1:6)], id.var="Lap")
names(DFMelt)[2] = "Rider"
names(DFMelt)[3] = "Time"
DFMelt$Lap = as.numeric(as.character(DFMelt$Lap))

DFMelt_NSL = melt(DF_LapTime_NSL[,c(1:6)], id.var="Lap")
...
ggplot(DFMelt, aes(x = Lap, y = Time, col= Rider, group = Rider)) +
  geom_point()+
  geom_xspline(spline_shape = -0.3)+
  xlab("Giro")+ ylab("Tempo (hh:mm:ss)")+
  ggtitle("DVV Koppenberg Tempo sul Giro (Full)")+
...
ggplot(DFMelt_NSL, aes(x = Lap, y = Time, col= Rider, group = Rider)) +
  ...
  geom_xspline(spline_shape = -0.3)+
  xlab("Giro")+ ylab("Tempo (hh:mm:ss)")+
  ggtitle("DVV Koppenberg Tempo sul Giro (No Lancio)")+
  scale_color_manual(values = c ("darkorange","chocolate","orangered1","red4", "black",
                                 "violetred", "darkviolet", "blue4", "blue2", "deepskyblue2"))+
  scale_x_continuous(breaks = 0:nlaps)
ggsave("RACE_NSL.jpg")
...
ggplot(DFMelt_TimeGap, aes(x = Lap, y = Gap, col= Rider, group = Rider)) +
  geom_point()+
  geom_xspline(spline_shape = -0.3)+
  xlab("Giro")+ ylab("Gap (s)")+
  ggtitle("DVV Koppenberg Gap Top5")+
  scale_color_manual(values = c ("darkorange","chocolate","orangered1","red4", "black",
                                 "violetred", "darkviolet", "blue4", "blue2", "deepskyblue2"))+
  scale_x_continuous(breaks = 0:nlaps)
ggsave("RACE_GAP.jpg")
